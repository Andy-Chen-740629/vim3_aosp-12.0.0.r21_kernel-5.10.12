...
extern void scan_sd_slot_bus(void);
...
dhd_wlan_init_gpio(wifi_adapter_info_t *adapter)
{
	...
	*/
	...
	{
//		gpio_wl_reg_on = -1;
		gpio_wl_reg_on = 442; // GPIOH_4
#ifdef CUSTOMER_OOB
//		gpio_wl_host_wake = -1;
		gpio_wl_host_wake = 447; // GPIOZ_15
#endif
	}

//-------------------------------------------------------------------
	err = gpio_request(411, "SD_TO_1V8");
	if (err < 0) {
		printf("%s: gpio_request switch to 1v8 failed\n", __FUNCTION__);
	}

	err = gpio_direction_output(411, 1);
	if (err < 0) {
		printf("%s: gpio(411) output high failed\n", __FUNCTION__);
	}
